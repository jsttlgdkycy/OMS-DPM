...
class DPMSolverSampler(object):
    ...
    def sample(self,
               ...
               t_start=None,
               t_end=None,
               # for multi model sampling
               use_model_schedule=False,
               model_schedule=None,
        ...
        if use_model_schedule:
            inference = lambda x, t, idx, c: self.model.apply_model_list(x, t, c, idx)  
            assert model_schedule is not None
        else:
            inference = lambda x, t, idx, c: self.model.apply_model(x, t, c)
        ...
        x, intermediates = dpm_solver.sample(img, t_start=t_start, t_end=t_end, steps=S, skip_type=skip_type, method=method, order=order, lower_order_final=lower_order_final, model_schedule=model_schedule, return_intermediate=True)
