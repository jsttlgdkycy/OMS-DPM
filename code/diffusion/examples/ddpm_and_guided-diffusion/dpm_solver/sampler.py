...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, model_idx, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, model_idx, **model_kwargs)
        else:
            output = model(x, t_input, model_idx, cond, **model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous, model_idx):
        """
        The noise predicition model function that is used for DPM-Solver.
        """
        if guidance_type == "uncond":
            return noise_pred_fn(x, t_continuous, model_idx)
        elif guidance_type == "classifier":
            assert classifier_fn is not None
            t_input = get_model_input_time(t_continuous)
            cond_grad = cond_grad_fn(x, t_input)
            sigma_t = noise_schedule.marginal_std(t_continuous)
            noise = noise_pred_fn(x, t_continuous, model_idx)
            return noise - guidance_scale * expand_dims(sigma_t, x.dim()) * cond_grad
        elif guidance_type == "classifier-free":
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(x, t_continuous, model_idx, cond=condition)
            else:
                x_in = torch.cat([x] * 2)
                t_in = torch.cat([t_continuous] * 2)
                c_in = torch.cat([unconditional_condition, condition])
                noise_uncond, noise = noise_pred_fn(x_in, t_in, model_idx, cond=c_in).chunk(2)
...
class DPM_Solver:
    def __init__(
        ...
        """
        self.model = lambda x, t, model_idx: model_fn(x, t.expand((x.shape[0])), model_idx)
    ...
    def noise_prediction_fn(self, x, t):
        ...
        """
        if getattr(self, "model_schedule", None) is not None:
            while self.model_schedule[self.model_idx]==0:
                self.model_idx -= 1
            output = self.model(x, t, self.model_idx)
            self.model_idx -= 1
        else:
    ...
    def get_orders_and_timesteps_for_model_schedule(self, model_schedule, skip_type, t_T, t_0, device):
        orders = []
        num_timesteps_outer = 0
        for i in range(len(model_schedule)//3):
            order = (torch.tensor(model_schedule[i*3:(i+1)*3])!=0).sum()
            if order!=0:
                orders.insert(0, order)
                num_timesteps_outer += 1
        if skip_type == 'logSNR':
            # To reproduce the results in DPM-Solver paper
            timesteps_outer = self.get_time_steps(skip_type, t_T, t_0, num_timesteps_outer, device)
        else:
            timesteps_outer = self.get_time_steps(skip_type, t_T, t_0, (torch.tensor(model_schedule)!=0).sum().item(), device)[torch.cumsum(torch.tensor([0,] + orders), 0).to(device)]
        return timesteps_outer, orders
    ...
    def sample(self, x, model_schedule=None, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        atol=0.0078, rtol=0.05, return_intermediate=False,
    ):
        """
        ...
        if model_schedule is not None:
            self.model_idx = len(model_schedule) - 1
        else:
            self.model_idx = -1
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                if model_schedule is not None:
                    timesteps_outer, orders = self.get_orders_and_timesteps_for_model_schedule(model_schedule=model_schedule, skip_type=skip_type, t_T=t_T, t_0=t_0, device=device)
                else:
                ...
                for step, order in enumerate(orders):
                    ...
                    print(f"Use {order}-th order | from Timestep {s} to Timestep {t}") 
