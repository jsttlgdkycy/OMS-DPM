...
def generalized_steps(x, seq, model_fn, b, eta=0, is_cond_classifier=False, classifier=None, classifier_scale=1.0, **model_kwargs):
    device = x.device
    with torch.no_grad():
        def model(x, t_discrete, model_idx):
            if is_cond_classifier:
                ...
                noise_uncond = model_fn(x, t_discrete, model_idx, **model_kwargs)
                ...
                return model_fn(x, t_discrete, model_idx, **model_kwargs)
        n = x.size(0)
...
def ddpm_steps(x, seq, model_fn, b, is_cond_classifier=False, classifier=None, classifier_scale=1.0, **model_kwargs):
    ...
    with torch.no_grad():
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            output = model(x, t.float(), model_idx)
            model_idx -= 1
            e = output
